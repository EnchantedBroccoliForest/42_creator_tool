...
- **Finalize** — the draft is converted into structured JSON matching the Outcome Token spawn shape: an array of outcomes (each with its own resolution criteria), start/end times in UTC, a short description, a compact `description` markdown payload, full resolution rules, edge cases, `is_early_resolution`, and `whitelisted: true`. Before emitting JSON, the finalizer prompt now runs through the 42 Market Creation Checklist: deterministic source, non-foregone outcome, exhaustive outcome set, professional non-`OT` names, standard markdown description, early-resolution flag, and the 30KB payload cap. A post-finalize validator (`src/util/finalMarketJson.js`) enforces the deterministic parts of that contract and force-sets frontend visibility. In the UI, the finalized output renders as a compact market card by default with a "Show full resolver spec" expansion; both views are copyable to clipboard.
...
- **Final-JSON structural validator** — `src/util/finalMarketJson.js` runs after the finalizer and prepares the compact ancillary payload. It rejects raw/unparseable finalizer text, missing/invalid `description`, non-standard resolution markdown, missing source links, non-boolean `is_early_resolution`, `whitelisted !== true`, payloads over 30KB, and outcomes whose `name` begins with the reserved `OT` prefix (Outcome Token identifier collision per the 42.space market creation guide). Failure surfaces as a hard error in both the UI and the orchestrator log rather than producing an unspawnable market.
