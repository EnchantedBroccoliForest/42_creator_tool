...
## Resolution Source
<name a PRIMARY source: link to a SPECIFIC page or endpoint that returns the resolution value — never a homepage — using [label](url) syntax, and state the UI parameters / filters / column needed to reproduce the value. If — and ONLY if — you have a substantively different (different host, mirror, or data feed) public-read fallback, name it as the SECONDARY source on its own line. If you do not have one, OMIT the secondary line entirely — do NOT emit a placeholder like "name a secondary source before submission". EVERY URL MUST be public-read: no api_key query parameters, no "Authorization:" / Bearer-token endpoints, no login or paywall pages. If the only authoritative source is API-key-gated, cite a public-read HTML page / on-chain feed / official press release instead.>
