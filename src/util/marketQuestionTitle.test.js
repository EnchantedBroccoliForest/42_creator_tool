...
describe('validateMarketQuestionTitle', () => {
  it('accepts short trader-facing market questions', () => {
    const result = validateMarketQuestionTitle('Kraken IPO by December 31, 2026?');

    expect(result.valid).toBe(true);
    expect(result.normalized).toBe('Kraken IPO by December 31, 2026?');
    expect(result.maxChars).toBe(getMarketQuestionTitleLimit());
  });

  it('normalizes whitespace before validating', () => {
    const result = validateMarketQuestionTitle('  Which   artist tops the 2026 Hot 100?  ');

    expect(result.valid).toBe(true);
    expect(result.normalized).toBe('Which artist tops the 2026 Hot 100?');
  });

  it('rejects titles that exceed the title budget', () => {
  ...
  it('rejects strings containing multiple questions', () => {
    const result = validateMarketQuestionTitle('Will A win? Will B win?');
  ...
  it('accepts bare uses of "resolve" in legitimate trader-facing questions', () => {
    ...
    for (const title of cases) {
      const result = validateMarketQuestionTitle(title);
  ...
  describe('resolver phrasing checks', () => {
    ...
    for (const { title, match } of verboseCases) {
      it(`rejects "${title}"`, () => {
        const result = validateMarketQuestionTitle(title);
        expect(result.valid).toBe(false);
    ...
    it('keeps accepting friendly trader-facing questions', () => {
      const cases = [
