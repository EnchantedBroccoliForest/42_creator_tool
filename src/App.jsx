                  ...
                  {finalContent.raw ? (
                    ...
                    <div className="final-doc final-doc--human">
                      ...
                      {finalContent.refinedQuestion && (
                        <section className="final-title-card stagger-item" style={{ '--stagger': 2 }}>
                          <div className="final-title-card__label-group">
                            <span className="final-title-card__label">{t('final.titleLabel')}</span>
                            <p className="final-title-card__text">{finalContent.refinedQuestion}</p>
                          </div>
                          <button
                            type="button"
                            className={`copy-btn ${copiedId === 'title' ? 'copy-btn--copied' : ''}`}
                            onClick={() => handleCopy(finalContent.refinedQuestion, 'title')}
                            title={t('final.copyTitleTooltip')}
                          >
                            {copiedId === 'title' ? t('common.copied') : t('final.copyTitle')}
                          </button>
                        </section>
                      )}

                      {finalContent.outcomes?.length > 0 && (
                        <section className="final-outcomes-card stagger-item" style={{ '--stagger': 3 }}>
                          <div className="final-outcomes-card__header">
                            <span className="final-outcomes-card__label">
                              {t('final.outcomesLabel', { n: finalContent.outcomes.length })}
                            </span>
                            <button
                              type="button"
                              className={`copy-btn ${copiedId === 'outcomes' ? 'copy-btn--copied' : ''}`}
                              onClick={() => handleCopy(
                                finalContent.outcomes
                                  .map((o, i) => `${i + 1}. ${o.name || ''}`.trim())
                                  .join('\n'),
                                'outcomes',
                              )}
                              title={t('final.copyOutcomesTooltip')}
                            >
                              {copiedId === 'outcomes' ? t('common.copied') : t('final.copyOutcomes')}
                            </button>
                          </div>
                          <ol className="final-outcomes-card__list">
                            {finalContent.outcomes.map((outcome, idx) => (
                              <li key={idx} className="final-outcomes-card__item">
                                <span className="final-outcomes-card__num">{idx + 1}</span>
                                <span className="final-outcomes-card__name">{outcome.name || t('common.none')}</span>
                              </li>
                            ))}
                          </ol>
                        </section>
                      ...
                      {resolutionDescriptionMarkdown && (
                        <section ref={finalOutputRef} className="final-output stagger-item" style={{ '--stagger': 4 }}>
                          ...
                          {/* UI-only disclaimer — intentionally NOT part of the
                              copied markdown so it never lands in 42.space's
                              ancillary data. Reminds the user to open each
                              Resolution Source link in their browser before
                              submitting. */}
                          <p className="final-output__disclaimer" role="note">
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                              <path d="M10.29 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z" />
                              <line x1="12" y1="9" x2="12" y2="13" />
                              <line x1="12" y1="17" x2="12.01" y2="17" />
                            </svg>
                            <span>{t('final.sourceDisclaimer')}</span>
                          </p>
                        </section>
                      )}

                      <details className="final-doc__details stagger-item" style={{ '--stagger': 5 }}>
